...
export default function WarehouseManagement({ subTab, lang = "vi", userRole, username }: WarehouseManagementProps) {
  ...
  // Sell product modal states for staff sales feature
  const [sellingProduct, setSellingProduct] = useState<Product | null>(null);
  const [sellQty, setSellQty] = useState<number>(1);
  const [sellCustomerName, setSellCustomerName] = useState<string>("");
  const [sellPaymentMethod, setSellPaymentMethod] = useState<string>("Chuyển khoản");
  ...
  const handleSellProduct = async () => {
    ...
    if (sellQty <= 0) {
      alert(lang === "vi" ? "Số lượng phải lớn hơn 0" : "Quantity must be greater than 0");
      return;
    }
    if (sellQty > sellingProduct.stock) {
      alert(lang === "vi" ? "Số lượng bán vượt quá hàng tồn trong kho!" : "Quantity exceeds stock level!");
      return;
    }
    ...
    try {
      const payload = {
        ...
        total: sellingProduct.price * sellQty,
        createdBy: username || "staff",
        customerName: sellCustomerName.trim() || (lang === 'vi' ? "Khách bán lẻ" : "Retail Customer"),
      ...
      if (res.ok) {
        alert(lang === "vi" ? "Bán hàng thành công!" : "Sales checked out successfully!");
        setSellingProduct(null);
        setSellQty(1);
        setSellCustomerName("");
        setSellPaymentMethod("Chuyển khoản");
        fetchData();
      } else {
        alert(lang === "vi" ? "Giao dịch bán hàng thất bại." : "Checkout transaction failed.");
      }
    } catch (err) {
      console.error(err);
      alert(lang === "vi" ? "Có lỗi xảy ra!" : "An error occurred!");
    }
  };
  ...
  return (
    <div className="w-full text-zinc-100 flex flex-col gap-6" id="warehouse-view-root">
                        ...
                        <div className="mt-5 flex gap-2 justify-end">
                          ...
                          {userRole === "ADMIN" ? (
                            <button
                              ...
                              className="p-2.5 rounded-lg bg-red-950/20 text-red-400/80 hover:text-red-400 hover:bg-red-950/40 transition-all font-display"
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-[#CCFF00] hover:bg-white text-black font-semibold transition-all flex items-center gap-1.5 cursor-pointer text-[10px] uppercase tracking-wider font-display shrink-0"
                              title={lang === "vi" ? "Bán sản phẩm trực tiếp ca làm" : "Sell product directly on duty"}
                            >
                              <ShoppingCart className="w-3.5 h-3.5" />
                              <span>{lang === "vi" ? "Bán Hàng" : "Sell"}</span>
                            </button>
                          )}
                        </div>
                      </div>
      ...
      {/* ======================= MODAL: DIRECT SELL PRODUCT (For Staff) ======================= */}
      <AnimatePresence>
        {sellingProduct && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            <motion.div 
              ...
              className="bg-zinc-950 border border-white/10 rounded-[2.5rem] p-8 w-full max-w-lg relative z-10 shadow-2xl overflow-hidden font-sans"
            >
              <div className="flex justify-between items-center mb-6 pb-3 border-b border-white/5">
                <h3 className="text-xl font-black italic uppercase text-white tracking-tight flex items-center gap-2 font-display">
                  <ShoppingCart className="w-5 h-5 text-[#CCFF00]" />
                  <span>{lang === 'vi' ? 'BÁN HÀNG TRỰC TIẾP' : 'POS DIRECT SALES'}</span>
                </h3>
                <button 
                  onClick={() => setSellingProduct(null)}
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex gap-4 items-center">
                  <div className="w-14 h-14 rounded-xl bg-neutral-800 shrink-0 overflow-hidden border border-white/5 flex items-center justify-center">
                    {sellingProduct.image ? (
                      <img src={sellingProduct.image} alt={sellingProduct.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    ) : (
                      <Package className="w-6 h-6 text-zinc-500" />
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    <span className="text-[9px] bg-white/10 text-[#CCFF00] font-mono font-bold px-1.5 py-0.5 rounded tracking-wide uppercase">
                      {sellingProduct.code}
                    </span>
                    <h4 className="text-sm font-bold text-white mt-1 truncate">{sellingProduct.name}</h4>
                    <p className="text-xs text-zinc-400 mt-0.5 font-mono">
                      {lang === 'vi' ? 'Đơn giá: ' : 'Unit Price: '}
                      <span className="text-[#CCFF00] font-mono font-bold">{sellingProduct.price.toLocaleString()}đ</span>
                    </p>
                    <p className="text-[10px] text-zinc-500 mt-0.5">
                      {lang === 'vi' ? `Tồn kho khả dụng: ${sellingProduct.stock}` : `Available Stock: ${sellingProduct.stock}`}
                    </p>
                  </div>
                </div>
                ...
                {/* Sell Quantity */}
                <div>
                  <label className="block text-[10px] font-mono text-zinc-400 uppercase tracking-widest mb-2 italic">
                    {lang === 'vi' ? 'Số lượng bán' : 'Sell Quantity'}
                  </label>
                  <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl p-1.5 max-w-[180px]">
                    <button
                      type="button"
                      onClick={() => setSellQty(Math.max(1, sellQty - 1))}
                      className="w-10 h-10 rounded-lg bg-white/5 hover:bg-[#CCFF00] hover:text-black transition-colors flex items-center justify-center font-bold text-lg select-none cursor-pointer"
                    >
                      -
                    </button>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setSellQty(Math.min(sellingProduct.stock, Math.max(1, val)));
                      }}
                      className="flex-1 w-full bg-transparent text-center focus:outline-none font-bold text-base font-mono text-white"
                    />
                    <button
                      type="button"
                      onClick={() => setSellQty(Math.min(sellingProduct.stock, sellQty + 1))}
                      className="w-10 h-10 rounded-lg bg-white/5 hover:bg-[#CCFF00] hover:text-black transition-colors flex items-center justify-center font-bold text-lg select-none cursor-pointer"
                    >
                      +
                    </button>
                ...
                <div>
                  <label className="block text-[10px] font-mono text-zinc-400 uppercase tracking-widest mb-2 italic">
                    {lang === 'vi' ? 'Tên khách hàng (Không bắt buộc)' : 'Customer Name (Optional)'}
                  </label>
                  <input
                    type="text"
                    placeholder={lang === 'vi' ? 'Ví dụ: Nguyễn Văn A, Khách vãng lai...' : 'e.g. Guest...'}
                    value={sellCustomerName}
                    onChange={(e) => setSellCustomerName(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 px-4 py-3 rounded-xl focus:border-[#CCFF00] outline-none text-xs text-white"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() => setSellPaymentMethod('Chuyển khoản')}
                      className={`py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all border cursor-pointer ${sellPaymentMethod === 'Chuyển khoản' ? 'bg-[#CCFF00] text-black border-[#CCFF00] shadow-[0_5px_15px_rgba(204,255,0,0.15)]' : 'bg-white/5 text-zinc-400 border-white/10'}`}
                    >
                      {lang === 'vi' ? 'Chuyển khoản' : 'Bank Transfer'}
                    </button>
                    <button
                      type="button"
                      onClick={() => setSellPaymentMethod('Tiền mặt')}
                      className={`py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all border cursor-pointer ${sellPaymentMethod === 'Tiền mặt' ? 'bg-zinc-100 text-black border-zinc-100 shadow-[0_5px_15px_rgba(255,255,255,0.1)]' : 'bg-white/5 text-zinc-400 border-white/10'}`}
                    >
                ...
                {/* Total */}
                <div className="bg-[#CCFF00]/5 border border-[#CCFF00]/15 rounded-2xl p-4 flex justify-between items-center">
                  <div>
                    <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider block">
                      {lang === 'vi' ? 'TỔNG CẦN THANH TOÁN' : 'TOTAL AMOUNT DUE'}
                    </span>
                    <span className="text-zinc-500 text-[9px] font-mono">
                      {sellQty} x {sellingProduct.price.toLocaleString()}đ
                    </span>
                  </div>
                  <div className="text-2xl font-black text-[#CCFF00] font-mono">
                    {(sellingProduct.price * sellQty).toLocaleString()}đ
                  </div>
                </div>
                ...
                <div className="flex gap-4 pt-4 border-t border-white/5">
                  <button
                    type="button"
                    onClick={() => setSellingProduct(null)}
                    className="flex-1 py-3.5 border border-white/10 hover:bg-white/5 rounded-xl text-xs font-black uppercase tracking-widest transition-all text-zinc-400 font-display cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-3.5 bg-[#CCFF00] hover:bg-white text-black rounded-xl text-xs font-black uppercase tracking-widest transition-all font-display hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-[#CCFF00]/15 cursor-pointer"
                  >
                    {lang === 'vi' ? 'XÁC NHẬN BÁN' : 'CONFIRM SALE'}
                  </button>
                </div>
              </div>
            </motion.div>
